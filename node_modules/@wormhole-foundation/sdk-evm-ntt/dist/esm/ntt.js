import { encoding, nativeChainIds, toChainId, } from "@wormhole-foundation/sdk-base";
import { canonicalAddress, serialize, toUniversal, universalAddress, } from "@wormhole-foundation/sdk-definitions";
...
import { loadAbiVersion, } from "./bindings.js";
export class EvmNttWormholeTranceiver {
    manager;
    address;
    transceiver;
    ...
    async *setPeer(peer) {
        const tx = await this.transceiver.setWormholePeer.populateTransaction(toChainId(peer.chain), universalAddress(peer));
        yield this.manager.createUnsignedTx(tx, "WormholeTransceiver.registerPeer");
    }
    async getPauser() {
        const pauser = await this.transceiver.pauser();
        return new EvmAddress(pauser);
    }
    ...
    async *setIsEvmChain(chain, isEvm) {
        const tx = await this.transceiver.setIsWormholeEvmChain.populateTransaction(toChainId(chain), isEvm);
        yield this.manager.createUnsignedTx(tx, "WormholeTransceiver.setIsEvmChain");
    }
    async *receive(attestation) {
        const tx = await this.transceiver.receiveMessage.populateTransaction(serialize(attestation));
        yield this.manager.createUnsignedTx(tx, "WormholeTransceiver.receiveMessage");
    }
    async isWormholeRelayingEnabled(destChain) {
        return await this.transceiver.isWormholeRelayingEnabled(toChainId(destChain));
    }
    async *setIsWormholeRelayingEnabled(destChain, enabled) {
        const tx = await this.transceiver.setIsWormholeRelayingEnabled.populateTransaction(toChainId(destChain), enabled);
    ...
    async isSpecialRelayingEnabled(destChain) {
        return await this.transceiver.isSpecialRelayingEnabled(toChainId(destChain));
    }
    async *setIsSpecialRelayingEnabled(destChain, enabled) {
        const tx = await this.transceiver.setIsSpecialRelayingEnabled.populateTransaction(toChainId(destChain), enabled);
...
export class EvmNtt {
    ...
    async getIsExecuted(attestation) {
        ...
        const isExecuted = await this.manager.isMessageExecuted(Ntt.messageDigest(attestation.emitterChain, payload["nttManagerPayload"]));
        if (!isExecuted)
            return false;
    ...
    getIsApproved(attestation) {
        const payload = attestation.payloadName === "WormholeTransfer"
            ? attestation.payload
            : attestation.payload["payload"];
        return this.manager.isMessageApproved(Ntt.messageDigest(attestation.emitterChain, payload["nttManagerPayload"]));
    }
    ...
    static async getVersion(provider, contracts) {
        const contract = new Contract(contracts.ntt.manager, ["function NTT_MANAGER_VERSION() public view returns (string)"], provider);
        try {
    ...
    async quoteDeliveryPrice(dstChain, options) {
        const [, totalPrice] = await this.manager.quoteDeliveryPrice(toChainId(dstChain), Ntt.encodeTransceiverInstructions(this.encodeOptions(options)));
        return totalPrice;
    }
    async *setPeer(peer, tokenDecimals, inboundLimit) {
        const tx = await this.manager.setPeer.populateTransaction(toChainId(peer.chain), universalAddress(peer), tokenDecimals, inboundLimit);
    ...
    async *transfer(sender, amount, destination, options) {
        ...
        const tokenContract = EvmPlatform.getTokenImplementation(this.provider, this.tokenAddress);
        ...
        const txReq = await this.manager
            ...
            .populateTransaction(amount, toChainId(destination.chain), receiver, receiver, options.queue, Ntt.encodeTransceiverInstructions(this.encodeOptions(options)), { value: totalPrice });
    ...
    async getInboundLimit(fromChain) {
        const encoded = (await this.manager.getInboundLimitParams(toChainId(fromChain))).limit;
        const trimmedAmount = decodeTrimmedAmount(encoded);
        const tokenDecimals = await this.getTokenDecimals();
        return untrim(trimmedAmount, tokenDecimals);
    }
    async *setInboundLimit(fromChain, limit) {
        const tx = await this.manager.setInboundLimit.populateTransaction(limit, toChainId(fromChain));
    ...
    async getInboundQueuedTransfer(fromChain, transceiverMessage) {
        const queuedTransfer = await this.manager.getInboundQueuedTransfer(Ntt.messageDigest(fromChain, transceiverMessage));
        if (queuedTransfer.txTimestamp > 0n) {
            ...
            return {
                recipient: new EvmAddress(recipient),
                amount: amount,
    ...
    async *completeInboundQueuedTransfer(fromChain, transceiverMessage, payer) {
        const tx = await this.manager.completeInboundQueuedTransfer.populateTransaction(Ntt.messageDigest(fromChain, transceiverMessage));
