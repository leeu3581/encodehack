...
class EvmTokenBridge {
    ...
    async getTokenNativeAddress(originChain, token) {
        return new sdk_evm_1.EvmAddress(token).toNative();
    }
    ...
    async *transfer(sender, recipient, token, amount, payload) {
        ...
        if ((0, sdk_connect_1.isNative)(token)) {
            const txReq = await (payload === undefined
                ? this.tokenBridge.wrapAndTransferETH.populateTransaction(recipientChainId, recipientAddress, sdk_evm_1.unusedArbiterFee, sdk_evm_1.unusedNonce, { value: amount + messageFee })
                : this.tokenBridge.wrapAndTransferETHWithPayload.populateTransaction(recipientChainId, recipientAddress, sdk_evm_1.unusedNonce, payload, { value: amount + messageFee }));
            ...
            const txReq = await (payload === undefined
                ? this.tokenBridge.transferTokens.populateTransaction(...sharedParams, sdk_evm_1.unusedArbiterFee, sdk_evm_1.unusedNonce, { value: messageFee })
                : this.tokenBridge.transferTokensWithPayload.populateTransaction(...sharedParams, sdk_evm_1.unusedNonce, payload, { value: messageFee }));
