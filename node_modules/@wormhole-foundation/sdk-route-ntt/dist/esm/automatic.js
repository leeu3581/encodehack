import { TransferState, Wormhole, amount, canonicalAddress, chainToPlatform, finality, isAttested, isDestinationQueued, isNative, isRedeemed, isSourceFinalized, isSourceInitiated, routes, signSendWait, } from "@wormhole-foundation/sdk-connect";
import "@wormhole-foundation/sdk-definitions-ntt";
import { NttRoute } from "./types.js";
export function nttAutomaticRoute(config) {
    class NttRouteImpl extends NttAutomaticRoute {
        static config = config;
    }
    return NttRouteImpl;
}
export class NttAutomaticRoute extends routes.AutomaticRoute {
    ...
    async isAvailable(request) {
        const { srcContracts } = NttRoute.resolveNttContracts(this.staticConfig, request.source.id, request.destination.id);
    ...
    async validate(request, params) {
        ...
        const gasDropoff = amount.parse(options.gasDropoff ?? "0.0", request.toChain.config.nativeTokenDecimals);
    ...
    async quote(request, params) {
        ...
        const result = {
            ...
            relayFee: {
                ...
                amount: amount.fromBaseUnits(deliveryPrice, fromChain.config.nativeTokenDecimals),
            },
            destinationNativeGas: amount.fromBaseUnits(params.normalizedParams.options.gasDropoff ?? 0n, toChain.config.nativeTokenDecimals),
