...
import { getNttProgram, getTransceiverProgram, } from "../lib/bindings.js";
...
export class SolanaNttWormholeTransceiver {
    ...
    async createReceiveIx(attestation, payer) {
        ...
        return this.program.methods
            ...
            vaa: utils.derivePostedVaaKey(this.manager.core.address, Buffer.from(attestation.hash)),
    ...
    async createBroadcastWormholeIdIx(payer, config, wormholeMessage) {
        const whAccs = utils.getWormholeDerivedAccounts(this.program.programId, this.manager.core.address);
        return this.program.methods
            .broadcastWormholeId()
            .accountsStrict({
            payer,
...
export class SolanaNtt {
    ...
    constructor(network, chain, connection, contracts, version = "3.0.0") {
        ...
        if ("wormhole" in contracts.ntt.transceiver &&
            ...
            transceiverTypes.map((transceiverType) => {
                ...
                const transceiverKey = new PublicKey(contracts.ntt.transceiver[transceiverType]);
                // handle emitterAccount case separately
                if (!PublicKey.isOnCurve(transceiverKey)) {
                    const whTransceiver = new SolanaNttWormholeTransceiver(this, getTransceiverProgram(connection, contracts.ntt.manager, version), version);
                    ...
                    this.transceivers.push(getTransceiverProgram(connection, contracts.ntt.transceiver[transceiverType], version));
                }
            });
        }
        ...
        if (this.contracts.ntt?.quoter)
            this.quoter = new NttQuoter(connection, this.contracts.ntt.quoter, this.contracts.ntt.manager);
        this.core = new SolanaWormholeCore(network, chain, connection, contracts);
    ...
    async getOwner() {
        const config = await this.getConfig();
        return new SolanaAddress(config.owner);
    }
    async getPauser() {
        return null;
    }
    ...
    async getTokenDecimals() {
        const config = await this.getConfig();
        return await SolanaPlatform.getDecimals(this.chain, this.connection, config.mint);
    }
    async getPeer(chain) {
    ...
    static async getVersion(connection, contracts, sender) {
        try {
            return await NTT.getVersion(connection, new PublicKey(contracts.ntt.manager), sender ? new SolanaAddress(sender).unwrap() : undefined);
        }
        catch (e) {
    ...
    async *initialize(sender, args) {
        ...
        const ix = await NTT.createInitializeInstruction(this.program, {
            ...args,
            payer,
            owner: payer,
            chain: this.chain,
    ...
    async *initializeOrUpdateLUT(args) {
        ...
        const ix = await NTT.initializeOrUpdateLUT(this.program, config, whTransceiverProgramId, {
            payer: args.payer,
    ...
    async *transfer(sender, amount, destination, options, outboxItem) {
        ...
        const transferIx = config.mode.locking != null
            ? NTT.createTransferLockInstruction(this.program, config, txArgs, this.pdas)
            : NTT.createTransferBurnInstruction(this.program, config, txArgs, this.pdas);
        ...
        if (options.automatic) {
            ...
            const relayIx = await this.quoter.createRequestRelayInstruction(payerAddress, outboxItem.publicKey, destination.chain, Number(fee) / LAMPORTS_PER_SOL, 
    ...
    async *createAta(sender) {
        ...
        if (acctInfo === null) {
            const transaction = new Transaction().add(splToken.createAssociatedTokenAccountInstruction(senderAddress, ata, senderAddress, config.mint, config.tokenProgram));
    ...
    async *redeem(attestations, payer) {
        ...
        for (const { attestation, ix } of attestations.map((attestation, ix) => ({
            ...
            if (ix === 0) {
                ...
                const redeemIx = NTT.createRedeemInstruction(this.program, config, whTransceiver.program.programId, {
                    payer: senderAddress,
                    vaa: wormholeNTT,
                });
    ...
    async *completeInboundQueuedTransfer(fromChain, transceiverMessage, payer) {
        ...
        const releaseArgs = {
            ...
            recipient: new PublicKey(transceiverMessage.payload.recipientAddress.toUint8Array()),
            chain: fromChain,
            // NOTE: this acts as `revertOnDelay` for versions < 3.x.x
            revertWhenNotReady: false,
        };
    ...
    async getInboundQueuedTransfer(chain, nttMessage) {
        ...
        if (inboxItem.releaseStatus.releaseAfter) {
            ...
            const xfer = {
                recipient: new SolanaAddress(recipientAddress),
