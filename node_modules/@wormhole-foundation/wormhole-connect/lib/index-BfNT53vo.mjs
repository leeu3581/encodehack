...
var Z = (n, f, e) => f in n ? Y(n, f, { enumerable: !0, configurable: !0, writable: !0, value: e }) : n[f] = e;
var d = (n, f, e) => Z(n, typeof f != "symbol" ? f + "" : f, e);
import { N as y, bg as f0, bh as W, bi as p, bj as e0, bk as a0, bl as t0, bm as _, bn as b0, bo as n0, bp as d0, b4 as L, bq as O, E as v, b as C, br as D, r as N, b8 as u, bs as q, bb as c0, bt as U, bd as H, be as j, b2 as s0, bu as i0, bv as I, b1 as S, $, bf as V, _ as G } from "./index-CGIqRhuR.mjs";
import "./index-Bfj3CEir.mjs";
const r0 = new RegExp("^bytes([0-9]+)$"), p0 = new RegExp("^(u?int)([0-9]*)$"), y0 = new RegExp("^(.*)\\[([0-9]*)\\]$");
function J(n, f, e) {
  switch (n) {
    case "address":
      return e ? p(_(f, 32)) : p(a0(f));
    case "string":
      return e0(f);
    case "bytes":
      return p(f);
    case "bool":
      return f = f ? "0x01" : "0x00", e ? p(_(f, 32)) : p(f);
  }
  let a = n.match(p0);
  if (a) {
    let t = a[1] === "int", b = parseInt(a[2] || "256");
    return y((!a[2] || a[2] === String(b)) && b % 8 === 0 && b !== 0 && b <= 256, "invalid number type", "type", n), e && (b = 256), t && (f = t0(f, b)), p(_(b0(f), b / 8));
  }
  if (a = n.match(r0), a) {
    const t = parseInt(a[1]);
    return y(String(t) === a[1] && t !== 0 && t <= 32, "invalid bytes type", "type", n), y(n0(f) === t, `invalid value for ${n}`, "value", f), e ? p(d0(f, 32)) : f;
  }
  if (a = n.match(y0), a && Array.isArray(f)) {
    const t = a[1], b = parseInt(a[2] || String(f.length));
    y(b === f.length, `invalid array length for ${n}`, "value", f);
    const c = [];
    return f.forEach(function(s) {
      c.push(J(t, s, !0));
    }), p(W(c));
  }
  y(!1, "invalid type", "type", n);
}
function o0(n, f) {
  y(n.length === f.length, "wrong number of values; expected ${ types.length }", "values", f);
  const e = [];
...
class E {
  constructor(f, e, a, t) {
    ...
    d(this, "contracts");
    d(this, "chainId");
    d(this, "circleChainId");
    d(this, "msgTransmitter");
    d(this, "tokenMessenger");
    d(this, "tokenEventHash");
    d(this, "messageSentEventHash");
    d(this, "messageReceivedEventHash");
    var i, r;
    if (this.network = f, this.chain = e, this.provider = a, this.contracts = t, f === "Devnet")
      throw new Error("CircleBridge not supported on Devnet");
    this.chainId = L.get(f, e);
    const b = O.get(f, e);
    if (b === void 0)
      throw new Error(`Circle chain id not found for ${f} ${e}`);
    this.circleChainId = b;
    const c = (i = t.cctp) == null ? void 0 : i.messageTransmitter;
    if (!c)
      throw new Error(`Circle Messenge Transmitter contract for domain ${e} not found`);
    this.msgTransmitter = k.connect(c, a);
    const s = (r = t.cctp) == null ? void 0 : r.tokenMessenger;
    if (!s)
      throw new Error(`Circle Token Messenger contract for domain ${e} not found`);
    this.tokenMessenger = R.connect(s, a), this.tokenEventHash = this.tokenMessenger.getEvent("DepositForBurn").fragment.topicHash, this.messageSentEventHash = this.msgTransmitter.getEvent("MessageSent").fragment.topicHash, this.messageReceivedEventHash = this.msgTransmitter.getEvent("MessageReceived").fragment.topicHash;
  }
  static async fromRpc(f, e) {
    const [a, t] = await v.chainFromRpc(f), b = e[t];
    if (b.network !== a)
      throw new Error(`Network mismatch: ${b.network} != ${a}`);
    return new E(a, t, f, b.contracts);
  }
  async *redeem(f, e, a) {
    const t = new C(f).toString(), b = await this.msgTransmitter.receiveMessage.populateTransaction(D.serialize(e), N.decode(a));
    yield this.createUnsignedTx(u(b, t), "CircleBridge.redeem");
  }
  //alternative naming: initiateTransfer
  async *transfer(f, e, a) {
    const t = new C(f).toString(), b = e.address.toUniversalAddress().toUint8Array(), c = q.get(this.network, this.chain), s = v.getTokenImplementation(this.provider, c);
    if (await s.allowance(t, this.tokenMessenger.target) < a) {
      const T = await s.approve.populateTransaction(this.tokenMessenger.target, a);
      yield this.createUnsignedTx(u(T, t), "ERC20.approve of CircleBridge", !1);
    }
    const r = await this.tokenMessenger.depositForBurn.populateTransaction(a, O.get(this.network, e.chain), b, c);
    yield this.createUnsignedTx(u(r, t), "CircleBridge.transfer");
  }
  async isTransferCompleted(f) {
    const e = c0(o0(["uint32", "uint64"], [f.sourceDomain, f.nonce]));
  ...
  async parseTransactionDetails(f) {
    ...
    const a = e.logs.filter((o) => o.topics[0] === this.messageSentEventHash).map((o) => {
      const { topics: Q, data: X } = o;
    ...
    a.length > 1 && console.error(`Expected 1 event to be found for transaction, got>${a.length}}`);
    const [t] = a, { message: b } = t.args, [c, s] = D.deserialize(N.decode(b)), { payload: i } = c, r = i.messageSender, T = i.mintRecipient, g = U(this.network, c.sourceDomain), A = U(this.network, c.destinationDomain), K = { chain: g, address: i.burnToken };
    return {
      from: { chain: g, address: r },
      to: { chain: A, address: T },
      token: K,
      amount: i.amount,
      message: c,
      id: { hash: s }
  ...
  createUnsignedTx(f, e, a = !1) {
    return new H(j(f, this.chainId), this.network, this.chain, e, a);
  }
}
class B {
  ...
  constructor(f, e, a, t) {
    ...
    this.chainId = L.get(f, e);
    const b = (s = this.contracts.cctp) == null ? void 0 : s.wormholeRelayer;
    if (!b)
      throw new Error(`Wormhole Circle relayer contract for domain ${e} not found`);
    this.circleRelayer = x.connect(b, a);
    const c = q.get(this.network, this.chain);
    if (!c)
  ...
  static async fromRpc(f, e) {
    ...
    if (b.network !== a)
      throw new Error(`Network mismatch: ${b.network} != ${a}`);
    return new B(a, t, f, b.contracts);
  }
  async getRelayerFee(f) {
    return this.circleRelayer.relayerFee(s0(f), this.tokenAddr);
  }
  async *transfer(f, e, a, t) {
    const b = new C(f).toString(), c = i0(e.chain), s = e.address.toUniversalAddress().toUint8Array(), i = t || 0n, r = v.getTokenImplementation(this.provider, this.tokenAddr);
    if (await r.allowance(b, this.circleRelayer.target) < a) {
      const A = await r.approve.populateTransaction(this.circleRelayer.target, a);
      yield this.createUnsignedTx(u(A, b), "ERC20.approve of CircleRelayer");
    }
    const g = await this.circleRelayer.transferTokensWithRelay.populateTransaction(this.tokenAddr, a, i, c, s);
    yield this.createUnsignedTx(u(g, b), "CircleRelayer.transfer");
  }
  createUnsignedTx(f, e, a = !1) {
    return new H(j(f, this.chainId), this.network, this.chain, e, a);
  }
  async nativeTokenAmount(f) {
    return await this.circleRelayer.calculateNativeSwapAmountOut(this.tokenAddr, f);
  }
  async maxSwapAmount() {
...
], F = "", u0 = (n) => n.length > 1;
class x extends I {
  constructor(...f) {
    u0(f) ? super(...f) : super(h, F, f[0]);
  }
  getDeployTransaction(f, e, a, t, b) {
    return super.getDeployTransaction(f, e, a, t, b || {});
  }
  deploy(f, e, a, t, b) {
    return super.deploy(f, e, a, t, b || {});
  }
  connect(f) {
...
const w = [
  ...
  getDeployTransaction(f, e, a) {
    return super.getDeployTransaction(f, e, a || {});
  }
  deploy(f, e, a) {
    return super.deploy(f, e, a || {});
  }
  connect(f) {
    return super.connect(f);
  }
...
const T0 = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  ...
  __proto__: null
}, Symbol.toStringTag, { value: "Module" })), v0 = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  CircleRelayer__factory: x,
  MessageTransmitter: T0,
  MessageTransmitter__factory: k,
  TokenMessenger: g0,
  TokenMessenger__factory: R
}, Symbol.toStringTag, { value: "Module" }));
V(G, "CircleBridge", E);
V(G, "AutomaticCircleBridge", B);
export {
  B as EvmAutomaticCircleBridge,
  E as EvmCircleBridge,
  v0 as ethers_contracts
};
