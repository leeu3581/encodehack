...
import { PublicKey as PublicKey8, SystemProgram as SystemProgram8 } from "@solana/web3.js";
...
function getEvmSwapParams(quote, destinationAddress, referrerAddress, signerAddress, signerChainId, payload) {
  ...
  const unwrapRedeem = quote.toToken.contract === ZeroAddress6;
  const criteria = {
    ...
    unwrap: unwrapRedeem,
    customPayload: payload ? `0x${Buffer10.from(payload).toString("hex")}` : "0x"
  };
...
function getSwapFromEvmTxPayload(quote, swapperAddress, destinationAddress, referrerAddresses, signerAddress, signerChainId, payload, permit) {
  ...
  const forwarderContract = new Contract4(addresses_default.MAYAN_FORWARDER_CONTRACT, MayanForwarderArtifact_default.abi);
  const mayanSwap = new Contract4(contractAddress, MayanSwapArtifact_default.abi);
  let forwarderMethod;
  let forwarderParams;
  let value;
  const _permit = permit || ZeroPermit;
  if (tokenIn === ZeroAddress6) {
...
import { SystemProgram as SystemProgram9 } from "@solana/web3.js";
import { ZeroAddress as ZeroAddress7 } from "ethers";
...
async function addInitOrderMoveCalls(quote, swapperAddress, destinationAddress, referrerAddress, mctpPackageId, feeManagerPackageId, suiClient, options) {
  ...
  const tokenOut = quote.toToken.contract === ZeroAddress7 ? nativeAddressToHexString(SystemProgram9.programId.toString(), getWormholeChainIdByName("solana")) : nativeAddressToHexString(quote.toToken.contract, quote.toToken.wChainId);
