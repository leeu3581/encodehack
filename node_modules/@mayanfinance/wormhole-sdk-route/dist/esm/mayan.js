import { fetchQuote, fetchTokenList, swapFromEvm, swapFromSolana, } from "@mayanfinance/swap-sdk";
import { TransferState, Wormhole, canonicalAddress, isSourceInitiated, isTokenId, routes, } from "@wormhole-foundation/connect-sdk";
import { NATIVE_CONTRACT_ADDRESS, getTransactionStatus, mayanEvmSigner, mayanSolanaSigner, toMayanChainName, } from "./utils";
export class MayanRoute extends routes.AutomaticRoute {
    ...
    async isAvailable() {
        ...
        try {
            ...
            const found = this.tokenList.find((token) => token.contract === tokenAddress || token.mint === tokenAddress);
            if (!found)
                throw new Error("Could not find token in token list");
        }
        catch (e) {
            console.error(e);
            return false;
    ...
    async quote(params) {
        ...
        const destTokenAddress = destination && isTokenId(destination.id)
    ...
    async initiate(signer, params) {
        ...
        try {
            ...
            if (this.request.from.chain === "Solana") {
                ...
                const txres = await swapFromEvm(quote, destinationAddress, params.options.deadlineInSeconds, undefined, rpc, mayanEvmSigner(signer));
                txhash = txres.hash;
            }
