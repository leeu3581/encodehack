...
export class BitgetWallet implements AdapterPlugin {
  ...
  deeplinkProvider(data?: { url?: string }): string {
    ...
    const url = data?.url || downloadLink; //window?.location?.href;
  ...
  async connect(): Promise<AccountInfo> {
    try {
      ...
      if (!addressInfo) throw `${BitgetWalletName} Address Info Error`;
      return addressInfo;
    } catch (error: any) {
      throw error;
    }
  ...
  async account(): Promise<AccountInfo> {
    ...
    if (!response) throw `${BitgetWalletName} Account Error`;
    return response;
  }
  ...
  async onNetworkChange(callback: any): Promise<void> {
    try {
      const handleNetworkChange = async (
        ...
        // This doesn't currently apply, since non-mainnet networks are not supported on Bitget.
        callback({
