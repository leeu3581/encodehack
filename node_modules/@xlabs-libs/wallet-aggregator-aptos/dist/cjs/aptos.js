"use strict";
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.AptosWallet = void 0;
const wallet_adapter_core_1 = require("@aptos-labs/wallet-adapter-core");
const aptos_wallet_adapter_1 = require("@bitget-wallet/aptos-wallet-adapter");
const aptos_wallet_adapter_2 = require("@martianwallet/aptos-wallet-adapter");
const wallet_adapter_plugin_1 = require("@pontem/wallet-adapter-plugin");
const fewcha_plugin_wallet_adapter_1 = require("fewcha-plugin-wallet-adapter");
const petra_plugin_wallet_adapter_1 = require("petra-plugin-wallet-adapter");
// FIXME: These wallets are not working
// import { OKXWallet } from "@okwallet/aptos-wallet-adapter";
// import { MSafeWalletAdapter } from "@msafe/aptos-wallet-adapter";
// import { TrustWallet } from "@trustwallet/aptos-wallet-adapter";
const wallet_aggregator_core_1 = require("@xlabs-libs/wallet-aggregator-core");
const NIGHTLY_WALLET = {
    icon: "data:image/svg+xml;base64,",
};
const SNAP_WALLET = {
    icon: "data:image/png;base64,",
};
const BITGET_WALLET = {
    icon: "data:image/svg+xml;base64,",
    name: "Bitget Wallet",
};
const OVERRIDES = {
    nightly: NIGHTLY_WALLET,
    snap: SNAP_WALLET,
    bitkeep: BITGET_WALLET,
    bitget: BITGET_WALLET,
};
/**
 * Attempt to get an override for the given adapter.
 *
 * @param adapter Aptos adapter to get the override for or default to itself
 * @returns the current adapter or the override if it exists
 */
const getOverride = (adapter) => OVERRIDES[adapter.name.toLocaleLowerCase()] || adapter;
/**
 * Looks for a property in the overrides and returns it if it exists, otherwise returns the property from the adapter.
 *
 * @param propertyName property name to look for
 * @param overrides overrides to inspect for the property
 * @param adapter adapter to default to if the property is not found in the overrides
 * @returns the property value from the overrides if it exists, otherwise the property value from the adapter
 */
const getPropertyByName = (propertyName, overrides, adapter) => (overrides[propertyName] || adapter[propertyName]);
/**
 * Looks for a property value if overrides exist return it otherwise defaults to adapter value.
 * @param propertyName property name to look for
 * @param adapter adapter to default to if the property is not found in the overrides
 * @returns the property value
 */
const getWalletOverride = (propertyName, adapter) => getPropertyByName(propertyName, getOverride(adapter), adapter);
class AptosWallet extends wallet_aggregator_core_1.Wallet {
    /**
     * @param selectedAptosWallet The Aptos wallet adapter which will serve as the underlying connection to the wallet
     * @param walletCore WalletCore class obtained via walletCoreFactory static function
     */
    constructor(selectedAptosWallet, walletCore) {
        super();
        this.selectedAptosWallet = selectedAptosWallet;
        this.walletCore = walletCore;
    }
    /**
     * @param config WalletCore configuration
     * @param withNonStandard Add nonstandard wallets to the wallet core, these includes the following wallets:
     * - BitgetWallet
     * - MartianWallet
     * - MSafeWalletAdapter
     * - OKXWallet
     * - PontemWallet
     * - TrustWallet
     * - FewchaWallet
     * - PetraWallet
     * @param newWalletsToAdd Add new wallets to the wallet core
     * @returns {WalletCore} WalletCore instance
     */
    static walletCoreFactory(config = { network: "mainnet" }, withNonStandard = true, newWalletsToAdd = []) {
        const nonStandardWallets = [
            // We are forcing PetraWallet to avoid the NotDetected issue
            new petra_plugin_wallet_adapter_1.PetraWallet(),
            // ---------------------------------------------------------
            new aptos_wallet_adapter_1.BitgetWallet(),
            new aptos_wallet_adapter_2.MartianWallet(),
            new wallet_adapter_plugin_1.PontemWallet(),
            // FIXME: These wallets are not working!!
            new fewcha_plugin_wallet_adapter_1.FewchaWallet(),
            // new OKXWallet(),
            // new MSafeWalletAdapter(),
            // new TrustWallet() as IAptosWallet,
        ];
        return new wallet_adapter_core_1.WalletCore(withNonStandard
            ? [...nonStandardWallets, ...newWalletsToAdd]
            : newWalletsToAdd, 
        // FIXME: T Wallet is not working and is removed from available wallets.
        [
            "Nightly",
            "Continue with Google",
            "Continue with Apple",
            "Mizu Wallet",
            "Pontem Wallet",
        ], config, true);
    }
    getName() {
        return getWalletOverride("name", this.selectedAptosWallet);
    }
    getUrl() {
        return this.selectedAptosWallet.url;
    }
    connect() {
        return __awaiter(this, void 0, void 0, function* () {
            var _a;
            yield this.walletCore.connect(this.selectedAptosWallet.name);
            // Set address
            this.address = (_a = this.walletCore.account) === null || _a === void 0 ? void 0 : _a.address;
            this.walletCore.on("accountChange", (accountInfo) => __awaiter(this, void 0, void 0, function* () {
                this.address = accountInfo === null || accountInfo === void 0 ? void 0 : accountInfo.address;
            }));
            // Set network
            this.network = this.walletCore.network || undefined;
            this.walletCore.on("networkChange", (network) => __awaiter(this, void 0, void 0, function* () {
                this.network = network || undefined;
            }));
            return this.getAddresses();
        });
    }
    getNetworkInfo() {
        return this.network;
    }
    isConnected() {
        return this.walletCore.isConnected();
    }
    disconnect() {
        this.walletCore.off("accountChange");
        this.walletCore.off("networkChange");
        return this.walletCore.disconnect();
    }
    getChainId() {
        return wallet_aggregator_core_1.CHAIN_ID_APTOS;
    }
    getAddress() {
        return this.address;
    }
    getAddresses() {
        const address = this.getAddress();
        return address ? [address] : [];
    }
    setMainAddress() {
        throw new wallet_aggregator_core_1.NotSupported();
    }
    getBalance() {
        throw new wallet_aggregator_core_1.NotSupported();
    }
    signTransaction(tx) {
        return __awaiter(this, void 0, void 0, function* () {
            return this.walletCore.signTransaction(tx);
        });
    }
    sendTransaction(txInput) {
        return __awaiter(this, void 0, void 0, function* () {
            const result = yield this.walletCore.submitTransaction({
                transaction: txInput.transaction,
                senderAuthenticator: txInput.senderAuthenticator,
            });
            return {
                id: result.hash,
                data: { hash: result.hash },
            };
        });
    }
    signAndSendTransaction(tx) {
        return __awaiter(this, void 0, void 0, function* () {
            const result = yield this.walletCore.signAndSubmitTransaction(tx);
            return {
                id: result.hash,
                data: result,
            };
        });
    }
    signMessage(msg) {
        return __awaiter(this, void 0, void 0, function* () {
            return this.walletCore.signMessage(msg);
        });
    }
    getIcon() {
        return getWalletOverride("icon", this.selectedAptosWallet);
    }
    getWalletState() {
        const state = this.selectedAptosWallet.readyState;
        return wallet_aggregator_core_1.WalletState[state || wallet_aggregator_core_1.WalletState.NotDetected];
    }
    getFeatures() {
        return Object.values(wallet_aggregator_core_1.BaseFeatures);
    }
    supportsChain(chainId) {
        return chainId === wallet_aggregator_core_1.CHAIN_ID_APTOS;
    }
}
exports.AptosWallet = AptosWallet;
//# sourceMappingURL=aptos.js.map