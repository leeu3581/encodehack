...
import { BaseFeatures, CHAIN_ID_ETH, NotConnected, NotSupported, Wallet, WalletState, isEVMChain, } from "@xlabs-libs/wallet-aggregator-core";
...
export class EVMWallet extends Wallet {
    ...
    async connect({ evmChainId } = {}) {
        ...
        this.provider = new ethers.BrowserProvider((await this.connector.getProvider()), "any");
    ...
    async onChange(data) {
        ...
        if (data.account)
            await this.onAccountsChanged([data.account]);
    }
    /**
     ...
     */
    async setPrefferedChain(chainId) {
    ...
    async getBalance(assetAddress) {
        ...
        if (assetAddress) {
            ...
            const balance = await erc20.balanceOf(assetAddress);
